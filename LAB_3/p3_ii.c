//ake file input from file and sort it using selection sort and write the output to another file for all case also note time taken for each case 
//for 1000 elements
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
void selectionSort(int arr[], int n, const char* outputFile) {
    for (int i = 0; i < n - 1; i++) {
        int minIndex = i;
        for (int j = i + 1; j < n; j++) {
...
int main() {
    ...
    start = clock();
    selectionSort(arrWorst, n, "worst_case_output1000.txt");
    end = clock();
    double timeWorst = (double)(end - start) / CLOCKS_PER_SEC;

    start = clock();
    selectionSort(arrBest, n, "best_case_output1000.txt");
    end = clock();
    double timeBest = (double)(end - start) / CLOCKS_PER_SEC;

    start = clock();
    selectionSort(arrAverage, n, "average_case_output1000.txt");
    end = clock();
    double timeAverage = (double)(end - start) / CLOCKS_PER_SEC;

    printf("Time taken for(1000) worst case: %f seconds\n", timeWorst);
    printf("Time taken for(1000) best case: %f seconds\n", timeBest);
    printf("Time taken for(1000) average case: %f seconds\n", timeAverage);
